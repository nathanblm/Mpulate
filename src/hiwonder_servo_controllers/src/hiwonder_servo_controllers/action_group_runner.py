...
class ActionGroupRunner:
    ...
    """
        Check whether the file is a legal action group file
    """
    ...
    def process_action_group_run(self, goal):
        ...
        '''All action group files end with ".json". Try to run commands for action groups that do not end with ".json". If the command has parameters that need to be passed in, use the'|' symbol to separate the command from the parameter'''
        print(name)
        print(repeat)
        success = 0
        with self.lock:
